...
public class BusService {
	...
	public ResponseEntity<ResponseStructure<Bus>> saveBus(BusRequest busRequest, int admin_id) {
		...
		if (recAdmin.isPresent()) {
			...
			bus.setAvailableSeats(busRequest.getNumberOfSeats());
	...
	public Bus mapToBus(BusRequest busRequest) {
		return Bus.builder().bus_name(busRequest.getBus_name()).busNumber(busRequest.getBusNumber())
				.dateOfDeparture(busRequest.getDateOfDeparture()).from(busRequest.getFrom()).to(busRequest.getTo())
				.numberOfSeats(busRequest.getNumberOfSeats()).availableSeats(busRequest.getAvailableSeats()).costPerSeat(busRequest.getCostPerSeat()).build();
	}
